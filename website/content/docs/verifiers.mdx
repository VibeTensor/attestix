...
description: Attestix credentials are issued once and verify anywhere. Six independent verifier implementations share one conformance suite, offline, with no Python runtime and zero trust in the issuer.
---
...
| **JS / TS** | `npm install @vibetensor/attestix` | Live: [attestix-js](https://github.com/VibeTensor/attestix-js) |
| **Go** | `go get github.com/VibeTensor/attestix-go` | Publishing soon: [attestix-go](https://github.com/VibeTensor/attestix-go) |
| **Rust** | `cargo add attestix` | Publishing soon: [attestix-rs](https://github.com/VibeTensor/attestix-rs) |
...
result = CredentialService().verify_credential_external(vc)  # the VC JSON
# -> dict with result["valid"] and per-check breakdown
```
