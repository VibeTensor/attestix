...
**Base L2** - A Layer 2 Ethereum rollup built by Coinbase. Attestix anchors to Base L2 testnet (Sepolia) today via EAS due to sub-$0.01 gas costs and fast finality. Mainnet schema registration is planned for a future release.
...
**EAS (Ethereum Attestation Service)** - An open protocol for creating on-chain attestations on Ethereum and L2 networks. Attestix uses EAS on Base L2 testnet for tamper-proof anchoring.
