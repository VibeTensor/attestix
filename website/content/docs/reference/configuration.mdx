...
Blockchain anchoring is optional. Attestix currently anchors to Base L2 testnet (Sepolia). To enable it, you need a funded wallet on Base Sepolia. Mainnet anchoring will ship once the mainnet schema is registered.
...
The project includes `Dockerfile.test` which runs all 585 tests (7 skipped) across unit, e2e, and conformance benchmarks in a clean container:
