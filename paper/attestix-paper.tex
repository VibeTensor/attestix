...
\begin{itemize}
    ...
    \item An \textbf{open-source implementation}\footnote{Source code: \url{https://github.com/VibeTensor/attestix}} as an MCP server with 47 tools, demonstrating that comprehensive attestation can be delivered as a protocol-level service with sub-millisecond cryptographic operations.
...
All operations use \textbf{Ed25519} (EdDSA over Curve25519) exclusively, chosen for deterministic signatures, compact 64-byte output, timing-attack resistance, and widespread DID ecosystem adoption.
...
Attestix is implemented in Python 3.10+ using \texttt{FastMCP}. The codebase comprises 5,596 lines of production code across 31 files, 166 test cases, and 5 example scripts.
...
\begin{table}[htbp]
\centering
\caption{Operation latency (median, 1000 runs).}
...
Identity creation (UAIT) & $\sim$2 ms \\
Credential issuance (W3C VC) & $\sim$3 ms \\
Credential verification & $\sim$1 ms \\
UCAN token creation & $\sim$2 ms \\
Merkle root (100 leaves) & $\sim$5 ms \\
...
\begin{table}[htbp]
\centering
\caption{Standards and specifications.}
\label{tab:standards}
\begin{tabular}{ll}
\toprule
\textbf{Standard} & \textbf{Coverage} \\
\midrule
...
We presented Attestix, the first integrated attestation infrastructure for autonomous AI agents combining cryptographic identity, verifiable credentials, capability delegation, regulatory compliance, reputation scoring, provenance tracking, and blockchain anchoring in a single MCP server. The system implements twelve EU AI Act articles, provides sub-millisecond operations, and bridges emerging agent protocols with established trust standards. Attestix is available as open-source under Apache 2.0 at \url{https://github.com/VibeTensor/attestix} and on PyPI as \texttt{attestix}.
